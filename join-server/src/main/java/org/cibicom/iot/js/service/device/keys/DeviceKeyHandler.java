...
import org.cibicom.iot.js.data.job.JobType;
import org.cibicom.iot.js.data.job.RunningJob;
import org.cibicom.iot.js.data.keys.*;
import org.cibicom.iot.js.data.lrwan.MACVersion;
import org.cibicom.iot.js.data.user.User;
import org.cibicom.iot.js.service.device.IDevKeyIdService;
import org.cibicom.iot.js.service.device.IDeviceService;
import org.cibicom.iot.js.service.log.IAppSKeyReqLogService;
import org.cibicom.iot.js.service.log.IJoinLogService;
import org.cibicom.iot.js.service.lrwan.JoinReqFailedExc;
import org.cibicom.iot.js.service.utils.IRunningJobService;
...
public class DeviceKeyHandler {
    ...
    public Pair<Boolean, String> init(KeySpec rootKeySpec, KeySpec kek, String password, String credentialIdentifier, User user, Boolean exposed, MACVersion macVersion, Boolean forwardAppSKeyToNS) {
        ...
        if (!validated) {
            return new Pair<>(false, "DevEUI, Root Key(s), or KEK couldn't be validated. Non-hex characters exist, incorrect length, or non-compatible key types depending on MAC Version have been specified");
        }
        ...
        persistDevice(rootKeySpec, kek, exposed, user, macVersion, forwardAppSKeyToNS);
    ...
    public Pair<Boolean, String> init(List<KeySpec> rootKeySpecs, KeySpec kek, String password, String credentialIdentifier, User user, Boolean exposed, MACVersion macVersion, Boolean forwardAppSKeyToNS) {
        ...
        persistDevices(rootKeySpecs, kek, exposed, user, macVersion, forwardAppSKeyToNS);
    ...
    public Pair<Boolean, String> init(KeySpec rootKeySpec, KeySpec kek, KeyCredential keyCredential, User user, Boolean exposed, MACVersion macVersion, Boolean forwardAppSKeyToNS) {
        // No need to derive lifetime keys, since more than 1 KeySpec is passed in case of LoRaWAN 1.1 when initializing
        boolean validated = validateKeySpecs(new ArrayList<>(List.of(rootKeySpec)), kek, macVersion);
        if (!validated) {
            return new Pair<>(false, "DevEUI, Root Key(s), or KEK couldn't be validated. Non-hex characters exist, incorrect length, or non-compatible key types depending on MAC Version have been specified");
    ...
    // This can take a long time, if storing many keys why we need to initialize a job in the database if keySpecs > 100
    public Pair<Boolean, String> init(List<KeySpec> rootKeySpecs, KeySpec kek, KeyCredential keyCredential, User user, Boolean exposed, MACVersion macVersion, Boolean forwardAppSKeyToNS) {
        boolean validated = validateKeySpecs(rootKeySpecs, kek, macVersion);
        if (!validated) {
            return new Pair<>(false, "DevEUI, Root Key(s), or KEK couldn't be validated. Non-hex characters exist, incorrect length, or non-compatible key types depending on MAC Version have been specified");
    ...
    private void persistDevice(KeySpec rootKeySpec, KeySpec kek, Boolean exposed, User owner, MACVersion macVersion, Boolean forwardAppSKeyToNS) {
    ...
    private void persistDevices(List<KeySpec> keySpecs, KeySpec kek, Boolean exposed, User owner, MACVersion macVersion, Boolean forwardAppSKeyToNS) {
    ...
    public Pair<Boolean, String> update(String oldDevEUI, Device updatedDevice, KeySpec rootKeySpec, KeySpec kek, MACVersion fromMACVersion, MACVersion toMACVersion) {
        ...
        if (!validated) {
            return new Pair<>(false, "DevEUI, Root Key(s), or KEK couldn't be validated. Non-hex characters exist, incorrect length, or non-compatible key types depending on provided MAC Version");
        }
